...
* `internal/generator/gen_cpp_test.go::TestGenCpp_StringErrorReturn` hardcodes `MsgId 133` for the first user function (line 130) but `pkg/server/types.go` now defines `MsgUserStart = 140`. The test fails on every run; either update the assertion to `140` (matching `MsgUserStart`) or, better, derive the expected ID from the same constant the template uses. Pre-existing rot, surfaced 2026-05-17.
