...
### 23.2 Tunability
* **PARTIAL (2026-05-17):** `pkg/server/manager.go` — promoted the 30s cleanup tick and 60s TTL to `ChunkManager.CleanupInterval` and `ChunkManager.ChunkBufferTTL` fields backed by `DefaultCleanupInterval` / `DefaultChunkBufferTTL` constants. The cleanup loop reads them once at startup; mutate before traffic flows. **REMAINING:** thread these (and `MaxChunkBufferBytes` from §23.3) through `xll.yaml` → `internal/config` so deployments can tune without rebuilds. Single co-change cluster — do all three fields in one pass when the YAML wiring lands.
...
* **OPEN (surfaced 2026-05-17):** the `cmd/` integration tier (`TestGenerateStringArgRepro`, `TestRepro_MultipleAsync`, `TestRepro_MemoryLeak`, `TestRepro_NestedIPC_Corruption`, `TestGenerate_Fixes`) is broken on Windows because `setupMockFlatc` writes a batch-style script to `flatc.exe` — Windows refuses to load it as a PE binary (`%1 is not compatible with version of Windows`). Two paths to fix: (a) write to `flatc.cmd` / `flatc.bat` and adjust `EnsureFlatc` lookup so a `.cmd`/`.bat` is accepted; (b) drop a tiny real Win32 stub that prints `flatc version <X>`. Per Taskfile.yml these tests live in the `integration` tier — `verify` is unaffected — but the failures hide real regressions and should be fixed before relying on the integration tier in CI.
