...
// generatedCppSubdirs are the subdirectories of <package>/cpp whose entire
// contents this run rewrites: `src/` and `tools/` come from the embedded asset
// map, `include/` from the asset map plus flatc's schema_generated.h plus the
// (conditional) ribbon headers.
var generatedCppSubdirs = []string{"src", "include", "tools"}

// pruneGeneratedCpp deletes the generated C++ subtrees before they are
// rewritten, so a file that USED to be emitted does not survive into the build.
//
// The build compiles `file(GLOB "${CMAKE_CURRENT_SOURCE_DIR}/src/*.cpp")`
// (CMakeLists.txt.tmpl), so a leftover .cpp from a previous xll-gen version is
// not merely dead weight — it is still compiled and linked. When an asset is
// deleted or renamed upstream, the stale copy typically still defines the
// symbols its replacement now defines, and the user gets duplicate-symbol link
// errors (or, worse, links the OLD implementation) in a tree they never edited.
// Conditional emission has the same shape: turning `ribbon.enabled` off left
// the previous run's ribbon_xml.h in include/.
//
// Deleting these three is safe because everything in them is regenerated by
// this same call and none of it is user-authored: the generated package
// directory is gitignored wholesale (`{{.Package}}/` in gitignore.tmpl) and the
// CMake build tree lives in `build/cpp`, outside it. Nothing else under cppDir
// is touched.
func pruneGeneratedCpp(cppDir string) error {
	for _, sub := range generatedCppSubdirs {
		if err := os.RemoveAll(filepath.Join(cppDir, sub)); err != nil {
			return fmt.Errorf("prune stale generated C++ in %s/: %w", sub, err)
		}
	}
	return nil
}
