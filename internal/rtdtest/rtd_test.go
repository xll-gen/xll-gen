...
`
	...
	err := sugar.Do(func(ctx *sugar.Context) error {
		...
		for i := 0; i < 30; i++ {
			...
			t.Logf("Tick %d: Value = %s", i, strVal)

			if strVal == "Connecting..." || strVal == "<nil>" || strVal == "" || strVal == "0" {
				continue
			}
			...
			if strVal != initialVal && strings.Contains(strVal, "AAPL") {
				t.Logf("SUCCESS: Value updated from '%s' to '%s'", initialVal, strVal)
				updated = true
				break
			}
		}
		...
		if !updated {
			return fmt.Errorf("RTD value did not update in time")
		}
		return nil
	})
